import {decodeData, generateQRBase64, generateQRData} from "../src";
...
test("should return base64 encoded QR for given data", async () => {
    ...
    const actual = await generateQRBase64(data)
    expect(actual).toBe(expected);
});
...
test("should return base64 encoded QR for given data with header", async () => {
    ...
    const actual = await generateQRBase64(data,ECC.M,header)
    expect(actual).toBe(expected);
});